...
void Tensor<float>::Padding(const std::vector<uint32_t>& pads,
  ...
  // 请补充代码
  // fcube 自动函数结束后释放内存
  // 不能直接对原始的mat作更改，已经fixed
  // uint32_t change_rows = this->rows()+pad_rows1+pad_rows1;
  // uint32_t change_cols = this->cols()+pad_cols1+pad_cols2;
  // uint32_t change_channels = this->channels();

  // //Tensor<float> f2(this->channels(), change_rows, change_cols);
  // for(int c=0; c<this->channels(); c++)
  // {
  //   auto ch_data = this->slice(c);
  //   //left
  //   arma::fmat tmp_cube1(this->rows(), pad_cols1, arma::fill::value(padding_value));
  //   ch_data.insert_cols(0, tmp_cube1);
    
  //   //right
  //   arma::fmat tmp_cube2(this->rows(), pad_cols2, arma::fill::value(padding_value));
  //   ch_data.insert_cols(ch_data.n_cols, tmp_cube2);
    
  //   //top
  //   arma::fmat tmp_cube3(pad_rows1, ch_data.n_cols, arma::fill::value(padding_value));
  //   ch_data.insert_rows(0, tmp_cube3);

  //   //bottom
  //   arma::fmat tmp_cube4(pad_rows2, ch_data.n_cols, arma::fill::value(padding_value));
  //   ch_data.insert_rows(ch_data.n_rows, tmp_cube4);
  //   std::cout<< "ch_data(" << c << "):"  << "\n" << ch_data << std::endl;
  // }
  // std::cout << "this->size(): " << this->size() << std::endl;
  // std::cout<< "shapes: "<< this->rows() << ","<<this->cols() << std::endl;


  // this->raw_shapes_ = {change_channels, change_rows, change_cols};
  // this->Reshape({change_channels, change_rows, change_cols}, true);
    // arma::cube A(2, 3, 4, arma::fill::randu);
    // std::cout << "A1:" << "\n" << A << std::endl;
    // arma::mat B = A.slice(1); // each slice is a matrix
    // std::cout << "B:" << "\n" << B << std::endl;
    // A.slice(0) = arma::randu<arma::mat>(2,3);
    // std::cout << "A2:" << "\n" << A << std::endl;

    // A.slice(0)(1,2) = 99.0;
    // std::cout << "A3:" << "\n" << A << std::endl;

    // A.subcube(0,0,1,  1,1,2) = arma::randu<arma::cube>(2,2,2);
    // std::cout << "A4:" << "\n" << A << std::endl;

    uint32_t rows = this->rows();
    uint32_t cols = this->cols();
    uint32_t channels = this->channels();
    auto new_data = arma::fcube(rows + pad_rows1 + pad_rows2, cols + pad_cols1 + pad_cols2, channels);
...
void Tensor<float>::Flatten(bool row_major) {
  CHECK(!this->data_.empty());
  // 请补充代码
  auto raw_channels = this->channels();
  auto raw_cols = this->cols();
  auto raw_rows = this->rows();
  this->raw_shapes_ = {(raw_cols * raw_rows * raw_channels)};
}
